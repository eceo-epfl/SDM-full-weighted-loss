#!/usr/bin/env python
"""
prg python package https://github.com/meeliskull/prg
Corrected some errors in prg.py (downloaded on 25/11/22)
np.alen() functions were replaced by shape or len, since np.alen is now deprecated
adapted precision_gain and recall_gain functions to handle either arrays or integers
"""
...
def precision_gain(tp, fn, fp, tn):
    ...
    if len(np.array(prec_gain).shape) != 0:
        if len(prec_gain) > 1:
            prec_gain[tn + fn == 0] = 0
        elif tn + fn == 0:
            prec_gain = 0
...
def _create_crossing_points(points, n_pos, n_neg):
    ...
    if points['recall_gain'][j] > 0:  # otherwise there is a point on the boundary and no need for a crossing point
        ...
        new_prec_gain = precision_gain(new_point[key_indices_1['TP']], new_point[key_indices_1['FN']], # here2
...
def create_prg_curve(labels, pos_scores, neg_scores=[]):
    ...
    if create_crossing_points == True:
        points = _create_crossing_points(points, n_pos, n_neg) # here 1
    else:
